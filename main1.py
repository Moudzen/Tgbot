...
def get_photo_loc(lat, lon):
    return f"https://yandex.ru/pogoda/maps/nowcast?lat={lat}&lon={lon}&via=hnav&le_Lightning=1"

def weather():
    owm = OWM('0f22e17db6aaaf7bbcbf3e1d3478ccd8')
    inf = owm.weather_manager().weather_at_place('Москва')
    weat = inf.weather
    temp = weat.temperature("celsius")
    return temp

async def weather(update, context):
    await update.message.reply_text(f"В Москве сейчас {round(weather()[0]["temp"])} градусов, чувствуется как {round(weather()[0]["feels_like"])} градусов")
